...
public class MemberSupportWebSocketHandler extends TextWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        if (currentUser == null || !UserScope.MEMBER.equals(currentUser.scope()) || !supportRealtimeService.registerMember(session, currentUser)) {
            session.close(CloseStatus.NOT_ACCEPTABLE.withReason("member token required"));
            return;
        }
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        try {
            ...
            SupportDispatchPayload payload = supportChatService.sendMemberMessage(currentUser, command.sessionId(), command.content());
